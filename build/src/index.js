...
function createLineChart(datos, colors) {
  ...
  function drawAxes(g) {
    ...
    const axisY = d3.axisLeft(scales.count.y).tickFormat(d3.format('d')).ticks(5).tickSizeInner(-width);
  ...
  function updateChart(datos, colors) {
    ...
    dataComb.forEach(d => {
      container.append('path').attr('class', 'book').style('stroke', () => d.color = color(d.key)).attr('d', line(d.values)).on('mouseover', () => {
        ...
        tooltip.style('opacity', 1).html(`<h3 class="tooltip-book-title">${d.values[0].title}</h3><p class="tooltip-book-text">Te ha costado ${d.values[0].days.toFixed(0)} días leerte las ${d.values[0].pages} páginas.<p/>`).style('left', postionWidthTooltip > w ? positionright : positionleft).style('top', `${d3.event.pageY - 28}px`);
      }).on('mouseout', () => {
...
function getRandomColor(elements) {
  const promiseColors = fetch(`http://www.colr.org/json/colors/random/${elements}`);
  promiseColors.then(response => response.json()).then(datos => {
    addElement(datos);
  }).catch(function (err) {
    console.error(err);
  });
