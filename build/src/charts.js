...
const color = d3.scaleOrdinal(arrayColors); //And finally I can create a chart, happy ends...
...
function scatterBooks(data) {
  ...
  function setupScales(dataz) {
    const countX = d3.scaleTime().domain(d3.extent(dataz, d => d.read));
    const countY = d3.scaleLinear().domain([d3.min(dataz, d => d.pages), d3.max(dataz, d => d.pages)]);
    scales.count = {
      x: countX,
      y: countY
    };
  ...
  function drawAxes(g) {
    ...
    const axisY = d3.axisLeft(scales.count.y).tickFormat(d => `${d} pp.`).ticks(5).tickPadding(7).tickSizeInner(-width);
  ...
  function updateChart(data) {
    ...
    const newLayer = layer.enter().append('circle').attr('class', 'scatter-circles');
    layer.merge(newLayer).attr('cx', d => scales.count.x(d.read)).attr('cy', d => scales.count.y(d.pages)).attr('r', 5);
    drawAxes(g);
  }
...
function createTimeLine(datos) {
  ...
  const svg = chart.select('svg');
  const scales = {};
  const tooltip = d3.select('.ringlera-chart-read').append('div').attr('class', 'tooltip tooltip-book').style('opacity', 0);
...
function barChartPages(dataz) {
  ...
  function setupScales(dataz) {
    ...
    const countY = d3.scaleLinear().domain([0, d3.max(dataz, d => d.value * 1.5)]);
    scales.count = {
      x: countX,
      y: countY
    };
  ...
  function drawAxes(g) {
    const axisX = d3.axisBottom(scales.count.x).tickValues(scales.count.x.domain().filter((d, i) => !(i % 5))).tickPadding(5);
    g.select('.axis-x').attr('transform', `translate(0,${height})`).call(axisX);
    const axisY = d3.axisLeft(scales.count.y).tickFormat(d3.format('d')).ticks(5).tickSize(-width).tickPadding(5);
    g.select('.axis-y').transition().duration(500).ease(d3.easeLinear).call(axisY);
  }
  ...
  function updateChart(dataz) {
    ...
    const layer = container.selectAll('.bar-vertical').remove().exit().data(dataz);
    const newLayer = layer.enter().append('rect').style('fill', d => {
      let valuesDate = d.key.split('-');
      let year = valuesDate[1];
      return d.color = color(year);
    });
    layer.merge(newLayer).attr('width', scales.count.x.bandwidth()).attr('x', d => scales.count.x(d.key)).attr('y', height).transition().duration(500).ease(d3.easeLinear).attr('width', scales.count.x.bandwidth()).attr('x', d => scales.count.x(d.key)).attr('y', d => scales.count.y(d.value)).attr('height', d => height - scales.count.y(d.value));
    drawAxes(g);
  }
...
function sliderD3(books) {
  ...
  let step = 1;
  const svg = d3.select('#slider').attr('width', width).attr('height', height);
  const slider = svg.append('g').classed('slider', true).attr('transform', `translate(${margin.left}, ${height / 2})`); // using clamp here to avoid slider exceeding the range limits

  const xScale = d3.scaleLinear().domain(range).range([0, width - margin.left - margin.right]).clamp(true); // array useful for step sliders
  ...
  xScale.clamp(true); // drag behavior initialization

  const drag = d3.drag().on('start.interrupt', function () {
    slider.interrupt();
  }).on('start drag', function () {
    dragged(d3.event.x);
  }); // initial transition

  slider.transition().duration(750).tween("drag", function () {
    dragged(xScale(yearMin));
  }); // this is the main bar with a stroke (applied through CSS)

  const track = slider.append('line').attr('class', 'track').attr('x1', xScale.range()[0]).attr('x2', xScale.range()[1]);
  const ticks = slider.append('g').attr('class', 'ticks').attr('transform', 'translate(0, 4)').call(xAxis); // drag handle

  const handle = slider.append('circle').classed('handle', true).attr('r', 8); // this is the bar on top of above tracks with stroke = transparent and on which the drag behaviour is actually called
  // try removing above 2 tracks and play around with the CSS for this track overlay, you'll see the difference

  const trackOverlay = d3.select(slider.node().appendChild(track.node().cloneNode())).attr('class', 'track-overlay').call(drag);
