(function (factory) {
	typeof define === 'function' && define.amd ? define(['core-js/modules/es.array.concat', 'core-js/modules/es.array.filter', 'core-js/modules/es.array.for-each', 'core-js/modules/es.array.iterator', 'core-js/modules/es.date.to-string', 'core-js/modules/es.number.to-fixed', 'core-js/modules/es.object.keys', 'core-js/modules/es.object.to-string', 'core-js/modules/es.object.values', 'core-js/modules/es.promise', 'core-js/modules/es.regexp.exec', 'core-js/modules/es.string.split', 'core-js/modules/web.dom-collections.for-each', 'core-js/modules/web.dom-collections.iterator'], factory) :
	factory();
}((function () { 'use strict';
	...
	function _getUserAsync() {
	  _getUserAsync = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
	    var response, data;
	...
	getUserAsync().then(function (data) {
	  var parseString = new DOMParser().parseFromString(data, 'application/xml');
	...
	function createLineChart(datos, colors) {
	  ...
	  var scales = {};
	  var tooltip = d3.select('.ringlera-chart').append('div').attr('class', 'tooltip tooltip-book').style('opacity', 0);
	  ...
	  function updateChart(datos, colors) {
	    ...
	    var line = d3.line().x(function (d) {
	      ...
	      return scales.count.y(d.pages);
	    });
	    container.selectAll('.line').remove().exit().data(dataComb);
	    dataComb.forEach(function (d) {
	      container.append('path').attr('class', 'book').style('stroke', function () {
	        return d.color = color(d.key);
	      }).attr('d', line(d.values)).on('mouseover', function () {
	        ...
	        tooltip.style('opacity', 1).html("<h3 class=\"tooltip-book-title\">".concat(d.values[0].title, "</h3><p class=\"tooltip-book-text\">Te ha costado ").concat(d.values[0].days.toFixed(0), " d\xEDas leerte las ").concat(d.values[0].pages, " p\xE1ginas.<p/>")).style('left', postionWidthTooltip > w ? positionright : positionleft).style('top', "".concat(d3.event.pageY - 28, "px"));
	...
	function getRandomColor(elements) {
	  var promiseColors = fetch("http://www.colr.org/json/colors/random/".concat(elements));
	  promiseColors.then(function (response) {
	...
	function addElement(datos) {
	  ...
	  try {
	    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
	      ...
	      arrayColors.push("#".concat(item.hex));
	    }
	  } catch (err) {
	    _iterator2.e(err);
	  } finally {
